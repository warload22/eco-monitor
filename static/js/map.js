...
function initMap() {
    ...
    // Логирование изменения масштаба
    map.getView().on('change:resolution', function() {
        const zoom = map.getView().getZoom();
        console.log('[Map] Zoom:', Math.round(zoom * 10) / 10);
    });
    ...
    // Инициализировать погодные слои (температура + ветер)
    if (typeof инициализироватьПогодныеСлои === 'function') {
        console.log('[Map] Инициализация погодных слоёв...');
        инициализироватьПогодныеСлои(map);
    } else {
        console.warn('[Map] Функция инициализироватьПогодныеСлои не найдена');
    }
