...
def получить_сетку_температуры():
    """
    Получить сетку температур для тепловой карты
    ...
    try:
        ...
        min_lat = float(request.args.get('min_lat', 55.55))  # Юг Москвы
        max_lat = float(request.args.get('max_lat', 55.95))  # Север Москвы
        min_lon = float(request.args.get('min_lon', 37.35))  # Запад Москвы
        max_lon = float(request.args.get('max_lon', 37.85))  # Восток Москвы
        ...
        точки = генерировать_тестовую_сетку_температуры(
        ...
        return jsonify({
            ...
            'data': точки,
            'count': len(точки),
            'note': 'Тестовые данные для прототипа. В будущем будут использоваться данные из метеомоделей.'
        }), 200
    
    except ValueError as e:
        return jsonify({'error': f'Некорректные параметры: {str(e)}'}), 400
    except Exception as e:
        current_app.logger.error(f"Ошибка при генерации сетки температуры: {e}")
...
def получить_векторы_ветра():
    """
    Получить векторное поле ветра для визуализации
    ...
    try:
        ...
        векторы = генерировать_тестовые_векторы_ветра(
            min_lat, max_lat, min_lon, max_lon, grid_size
        )
        ...
        current_app.logger.error(f"Ошибка при генерации векторов ветра: {e}")
        return jsonify({'error': 'Ошибка сервера'}), 500
